...
(function(){
	...
	{
		console.debug("begine to load " + typename + " in file " + filename);
		...
		var finishfunc = function(){
			...
			console.debug("finish one thread : " + finish_count);
			
			// 释放内存。
			//C3D.engine.Prepare();
			//System.ReleaseObject(iThreadReturnList[this.index]);
			//iThreadReturnList[this.index] = null;
			
			// 回调用户，进度条步进。
			console.debug("start to update progress.");
			onprogress(typename, {
				total : iThreadReturnList.length,
				loaded : finish_count
			});
			console.debug("end of updating progress.");
			
			if(this.bHasMoreNode)
			{
				console.debug("has more nodes to load");
				this.loadNextNodes();
				console.debug("finish load next node");
			}
			if(finish_count >= iThreadReturnList.length)
			{
				// 所有线程都加载完毕，让用户决定下一步进行什么。
				console.debug("finish all thread.");
				finish(typename);
			}
			
			console.debug("finish finish_function");
		}
		
		var func_loadNextNodes = function(){
			console.debug("func_loadNextNodes");
			var idx = iThreadReturnList.length;
			var start = iThreadReturnList.length;
			
			console.debug("length of iThreadReturnList is " + idx);
			
			// 遍历每个`<texture>`节点，并加载之。
			var MaxThread = 4//Global.MAX_THREAD;//xxdebug
			while(typeNodeItr.HasNext() && MaxThread > 0)
			{
				console.debug("typeNodeItr.HasNext() && MaxThread > 0");
				MaxThread--;
				// 获取`<texture>`节点
				...
				{
					// 创建临时节点，用于加载单个
					// typeNode = <library>/art/factories/gnd99.xml</library>
				...
				{
					// 创建临时节点，用于加载单个
					// tmpWorldNodes = <shader><file>/shader/ambient.xml</file></shader>
			...
			var bHasMoreNode = typeNodeItr.HasNext();
			
			console.debug("has node to load ? " + bHasMoreNode);
			
			for(var i = start; i < iThreadReturnList.length; i++)
			{
				console.debug("set finish function");
				var tmpObj = {
					index : i,
					bHasMoreNode : bHasMoreNode,
					loadNextNodes : func_loadNextNodes
				};
				iThreadReturnList[i].onfinish = spp.hitch(tmpObj,finishfunc);
				console.debug("finish set finish function");
			}
		}
		func_loadNextNodes();
	};
	...
	Loader.LoadSector = function(filename, typename, processWorldNode, finish)
	{
		...
		// 在<world>中，除了<sector>节点，其他都删除。
		{
			while(worldChildItr.HasNext())
			{
				...
				{
					console.debug("remove " + node.value);
		...
		{
			console.debug("finish load sector");
			finish(typename);
		};
	};
